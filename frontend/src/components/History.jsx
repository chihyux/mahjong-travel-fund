...
import { fmtMoney, fmtDate, playerName } from '../lib/utils';
...
export default function History() {
  ...
  const [tab, setTab] = useState('tsumos');
  const [confirmDel, setConfirmDel] = useState(null); // { type, id, label }

  const tsumoList = [...(tsumos || [])].sort((a, b) =>
    new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime()
  );
  const settleList = [...(settlements || [])].sort((a, b) =>
    new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime()
  );
                ...
                <div key={t.id} className="py-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[18px] font-medium">
                      ...
                      {t.count > 1 && <span className="text-ink-3 ml-1">× {t.count}</span>}
                    </div>
