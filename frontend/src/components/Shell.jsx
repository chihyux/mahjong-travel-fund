...
export default function Shell({ current, onNav, children }) {
  const { data, isAdmin, toast, actions } = useStore();
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left transition-colors ${
        active
          ? 'bg-sage/10 text-sage-deep font-medium'
          : 'text-ink-2 hover:bg-hint'
      }`}
    >
